let champions = ["아트록스", "Atrox", "Ahri", "Akali", "Alstar", "Amumu", "Anivia", "Annie", "Ashe"];

//
// function search_champion() {
//     // let search_result = document.getElementsByClassName('search_result');
//     // search_result.style.display = "block";
//     // window.alert(search_result);
//
//     // 검색어 받아오기
//     let input = document.getElementById('search_left').value;
//
//     // 검색어 소문자로 변경(영어인 경우)
...
// 검색바 - 온키로 작동
function auto_complete_left() {
    let arr = champions;    // 챔피언 리스트를 받음
    let current_focus;      // 키보드 움직임을 받음
    let input_text = document.getElementById('search_left').value;      // 검색어를 받음
    let parent_node = document.getElementById('autocomplete_left');      // parent node 찾기
    ...
    // console.log('parant node? ', parent_node);      // id=autocomplete
    // console.log("current fucus 는? ", current_focus);        // undefined

    close_lists(parent_node, input_text);      // 기존에 열려있던 결과 리스트 갱
    if (!input_text) {
        return false;
    }
    ...
    for (let i = 0; i < arr.length; i++) {
        if (arr[i].substr(0, input_text.length).toUpperCase() == input_text.toUpperCase()) {
            ...
            b.addEventListener("click", function () {
                ...
                search_button_left(clicked_champion);
            });
            a.appendChild(b);
        }
    }
...
function search_button_left(clicked_champion) {
    const search1 = document.getElementById("search_left");
    let input;
